...
from src.app.utils.minio_utils import get_s3, retrieve_files, upload_images
...
class Model(MLModel):
    ...
    async def process_images(
        ...
        img = img.resize((img.size[0] // 3, img.size[1] // 3), resample=Image.Resampling.LANCZOS)
    ...
    async def predict(
        ...
        dict_images = retrieve_files(s3=self._s3_client, imgs_path=image_path[0])
        ...
        if not dict_images:
            ...
            upload_images(s3=self._s3_client,  minio_path=settings.s3.folder + '/' + image_path[0] + "/" + settings.s3.result_folder, images={image_path[0]: output})
